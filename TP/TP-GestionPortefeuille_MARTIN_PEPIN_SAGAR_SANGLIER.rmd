...
this part on Black-Litterman Model, when we talk about "returns", we
...
Now, we'll add some information to the prior and suppose that we have
the following views on some portfolios (it can be summarized as
$P \mu = Q + \varepsilon^{(v)}$ where each row of $P$ being a portfolio,
$Q$ containing the returns expected by the analyst on these portfolios,
...
Canada, which is logic since they are expected to outperform according
to our views.

Note : in the examples shown in this section, the weights do not sum up
to $1$ as we did not defined such constraint when calculating $w^*$.
Indeed, it is not very important, as the goal of these examples is just
to show the impact of investor views on assets allocation.

### Comparison with BLCOP

Now, let's check that we retrieve the same results using directly the
BLCOP package.
...
# General function to compute the estimation of Sigma and mu using BLCOP
...
expect Canada to outperform USA by 4% instead of 3% previously). As a
result, the weight on the second portfolio has increased (from 0.418 to
0.538) since $\lambda$ is an increasing function of $q$ for a given view
(see property 3.2 in the paper).
...
distribution, we will compare the tangency portfolio allocation when
...
In the Mean-Variance framework, the tangency portfolio is the one that
...
portfolio on the CML is a combination of the tangency portfolio and the
riskless asset. Since we're interested in finding the tangency
portfolio, we can find it by solving for any portfolio on the CML with
an excess return wrt risk-free rate $\tilde{\mu_p}$ :
...
it is on the efficient frontier) but is also on the CML, we have for
this portfolio : $w^T \mathbf{1} = 1$. Thus, to find the tangency
portfolio, we just need to normalize the optimal weights found.
...
Note : as in this section we consider expected returns as excess returns
w.r.t. risk-free rate, there is no need here to define a risk-free rate
because the first constraint will be written as
...
```{r, fig.cap="Tangency Porfolio - comparing prior and ex-post distributions"}
...
US, a little more weight on Germany and less weight on the rest of
...
2.  Choose a 36-month interval and calculate the covariance matrix.
    ...
    corrections.
3.  Calculate the tangency portfolio and present the solution
    numerically and graphically. What do you think ?
...
riskless asset returns data is already expressed monthly and the dates
are in the format "YYYY-MM-01". Thus we need to change the dates of our
monthly returns to match the way the ones of the risk-free rates are
displayed, and select the 36 months corresponding for both datasets
(notice there are no missing values in riskfree rate dataset, nor in
monthly returns dataset by construction).
...
We will find the tangency portfolio as in Question 3 of the first
section, and once again here, we do not allow short sales. Thus, using
"quadprog" library, we have (see Question 3 of the first section for
details): $d = 0_{N,1}$, $b = w$, $D = \Sigma$,
...
```{r, fig.cap="Tangency Porfolio (Historical covariance) - bar chart"}
barplot(w_star, col="#265073",
        main = "Tangency Portfolio - Historical Cov.",
        xlab = "Assets", ylab = "Weight")
```

The weights of tangency portfolio found are diversified only across a
few assets.

Note : here we chose a general view which does not allow for a precise
way to see the assets in question. If we want to see the most important
assets in the portfolio (say those with a weight above $1$%), we can use
a pie chart :

```{r, fig.cap="Tangency Porfolio (Historical covariance) - pie chart"}
 
plot_pie_chart <- function(returns, w_star, title, threshold = 1e-2){
  # Keeping assets with weights above the threshold
  selected_assets <- colnames(returns)[w_star > threshold]
  selected_weights <- w_star[w_star > threshold]
  
  # Print the number of selected assets
  nb_selected_weights <- length(selected_assets)
  print(paste("Number of assets with a weight above", threshold, ":", nb_selected_weights))
  
  # Create pie chart for selected assets
  pie(selected_weights,
      labels = selected_assets,
      main = paste(title, "( Weights >", threshold,")"),
      col = terrain.colors(length(selected_weights)),
      cex = 0.8)
  
}

plot_pie_chart(month_rets_select, w_star, "Tangency Portfolio - Historical Cov.")
```
...
5.  Formulate and solve the quadratic program whose solution is the
    tangency portfolio, and compare this solution to the previous one.
...
associated eigenvalues, but also the Principal Components (scaled by the
...
```{r, fig.cap="PCA decomposition (first values)"}
...
Since we have calculated $f(t)$, we can estimate $\mu$ and $B$ by
Ordinary Least Squares (OLS) method as the noise is homoscedastic.
...
Analysis enables to project data in an orthogonal basis, we get
$\Sigma_f$ diagonal.
...
```{r, fig.cap="Tangency Porfolio (PCA factors) - bar chart"}
...
i.e. the solution is more robust. This is not surprising as an efficient portfolio in Mean-Variance theory is determined among others by $\Sigma^{-1}$, which is mainly function of lowest eigenvalues of $\Sigma$. However, when taking $\Sigma$ as the historical covariance matrix, then its lowest eigenvalues correspond to statistical noise and an efficient portfolio will be greatly affected by this statistical noise. 

Once again, to get a more concrete idea of the assets in question, we
can use a pie chart :

```{r, fig.cap="Tangency Porfolio (PCA factors) - pie chart"}
plot_pie_chart(month_rets_select, w_star, "Tangency Portfolio - PCA Factors")
```
...
selected. Moreover, the fama french returns data is already expressed
monthly, dates are already in the format "YYYY-MM-01", and there are no
missing values.
...
terms in the same way as previously. Notice that here, $\Sigma_f$ is not
diagonal anymore.
...
Now, let's find the tangency portfolio, using the "Diagonizable Model of
...
```{r, fig.cap="Tangency Porfolio (FF factors) - bar chart"}
...
We get the same conclusion when using a pie chart which also shows a
little over 25 assets with a weight above $1$% compared to only about 15
assets for the historical covariance.

```{r, fig.cap="Tangency Porfolio (FF factors) - pie chart"}
plot_pie_chart(month_rets_select, w_star, "Tangency Portfolio - FF Factors")
```
...
```{r, fig.cap="First PCA component compared with Fama-French market factor"}
...
This lab gave us the opportunity to show how the Black-Litterman model
is able to incorporate the investor's views by proceeding in the reverse
way the Markowitz model would. Indeed, here we used the observation of
the tangency portfolio to infer the expected return which lead to that
portfolio. As a result we were able to see portfolio redistribution when
taking into account the investor's views.

In the second part we tried to obtain more stable portfolios than when
only using the covariance matrix estimated on historical data. The issue
with this first approach is that it incorporates statistical noise which
distorts the results. We saw how using statistical factors as well as
the Fama-French factors allows to have more robust portfolios and how
they avoid such drawback.

**Note** : concerning LLM, they were not as proficient in R as they can
be in other languages such as Python. Moreover, the .Rmd version of
exercices solutions and of the slides available to us were great sources
of information. As a result, we ended up using LLMs on really rare
occasions and mainly focused on the files in IMT-GP-2024 and on the
rdocumentation site.
